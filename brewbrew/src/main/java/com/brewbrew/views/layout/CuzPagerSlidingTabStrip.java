...
package com.brewbrew.views.layout;
...
public class CuzPagerSlidingTabStrip extends HorizontalScrollView {
	...
	private int indicatorColor = 0xFF3F2925;
	private int underlineColor = 0x993F2925;
	...
	private View selectLayoutTab = null;

	private final int TAB_MAX_COUNT = 3;
	
	private boolean tab_new = false;

	public CuzPagerSlidingTabStrip(Context context) {
		this(context, null);
	}

	public CuzPagerSlidingTabStrip(Context context, AttributeSet attrs) {
	...
	public CuzPagerSlidingTabStrip(Context context, AttributeSet attrs,
		...
		/*
		 * dividerPaint = new Paint(); dividerPaint.setAntiAlias(true);
	...
	private void addIconTab(final int position, int resId) {
		...
		tab.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				// ���ý� ���� button selected flase ó��.
				if (selectLayoutTab != null)
					selectLayoutTab.setSelected(false);
	...
	private void addLayoutTab(final int position, int resId) {

        Log.i("jony","addLayoutTab : position "+ position+" resId "+resId);
		...
		if (position == 0) {
			if (selectLayoutTab == null) {
				selectLayoutTab = tab;
				selectLayoutTab.setSelected(true);
			}
		}
		...
		// add new marker S
        /*
		if( position == TAB_MAX_COUNT - 1)
		{
			ImageView iv = new ImageView(getContext());
			iv.setBackgroundResource(R.drawable.frm_frame_title_new);
			iv.setVisibility(View.GONE);
	
			tab.addView(iv);
			
			iv.setLayoutParams(getNewMarkerTabParam());
		}
		*/
		// add new marker E
	...
	private RelativeLayout.LayoutParams getNewMarkerTabParam()
	{
	...
	private void updateTabStyles() {
		...
		for (int i = 0; i < tabCount; i++) {
			...
			if (v instanceof TextView) {
				...
				int setPadding = (int) TypedValue.applyDimension(
						TypedValue.COMPLEX_UNIT_DIP, w / 6, dm);

				setPadding = w / 6;
				tab.setWidth(setPadding);
				...
				// event button area! S
				// int nEventBtnWidth = (int) TypedValue.applyDimension(
				// TypedValue.COMPLEX_UNIT_DIP, 50, dm);

				// w = w - nEventBtnWidth;
				// event button area! E

				int setSize = (int) TypedValue.applyDimension(
						TypedValue.COMPLEX_UNIT_DIP, w / TAB_MAX_COUNT, dm);

				setSize = w / TAB_MAX_COUNT;
				tab.setMinimumWidth(setSize);
				...
				if(i == TAB_MAX_COUNT - 1)
				{
					int cnt = tab.getChildCount();
					
					if( cnt > 0)
					{
						ImageView iv = (ImageView) tab.getChildAt(0);
	...
	public void selectTab(int position) {
		...
		if (v instanceof ImageButton) {
			...
			selectLayoutTab = (ImageButton) v;

		}else if (v instanceof RelativeLayout) {

			((RelativeLayout) v).setSelected(true);

			selectLayoutTab = (RelativeLayout) v;
	...
	public void setTabNewMarker(boolean flag) {
		
		if( this.tab_new != flag)
		{
			this.tab_new = flag;
			updateTabStyles();
		}
